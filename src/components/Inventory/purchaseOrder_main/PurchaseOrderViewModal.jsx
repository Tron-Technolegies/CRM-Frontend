...
function ErrorState({ onClose }) {
  return (
    <div className="py-12 flex flex-col items-center gap-3 text-center">
      ...
      <p className="text-sm font-medium text-[#111827]">Failed to load purchase order</p>
      <button
        type="button"
        onClick={onClose}
...
export default function PurchaseOrderViewModal({ open, onClose, onEdit, orderId = null }) {
  ...
  return (
    <Modal open={open} title="Purchase Order Details" subtitle="Full profile for this purchase order" onClose={onClose} maxWidthClassName="max-w-4xl">
      {loading && <Skeleton />}
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-4 p-5 rounded-2xl bg-gradient-to-br from-[#F5F0FF] to-white border border-[#E5D9FF]">
              ...
              <div className="flex-1 min-w-0 pt-0.5">
                <h3 className="text-xl font-bold text-[#0F172A] truncate">{data.subject || "Untitled Purchase Order"}</h3>
                <p className="text-sm text-[#6B7280] truncate mt-0.5 font-medium">
                  {data.purchaseOrderNumber} {data.vendor ? `· ${data.vendor}` : ""}
                </p>
            ...
            <Section title="Order Details">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
                <Field label="PO Number" icon={FileText} value={formatValue(data.purchaseOrderNumber)} />
                ...
                <Field label="Vendor" icon={Users} value={formatValue(data.vendor)} />
                <Field label="Owner" icon={Users} value={formatValue(data.owner)} />
                <Field label="Purchase Date" icon={Calendar} value={formatValue(data.purchaseDate)} />
                <Field label="Expected Delivery" icon={Calendar} value={formatValue(data.expectedDeliveryDate)} />
              </div>
            </Section>
            ...
            <AuditHistory
              ...
              objectId={data.id || orderId}
            />
          </div>

          <div className="mt-5 pt-4 border-t border-[#F0F2F5] flex items-center justify-between">
            <p className="text-xs text-[#9CA3AF]">Expected delivery {data.expectedDeliveryDate || "—"}</p>
