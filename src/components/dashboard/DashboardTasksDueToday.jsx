  ...
  useEffect(() => {
    api.get("/task/view/")
      .then((res) => {
        ...
        const dueTodayOrPending = res.data
  ...
  return (
    <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 h-full">
      ...
      <div className="mt-6 divide-y divide-[#EEF2F7] max-h-[400px] overflow-y-auto">
        {tasks.length === 0 && (
          <p className="text-sm text-[#64748B] py-4">No tasks due today.</p>
        )}
        {tasks.map((t) => (
          <div key={t.id} className="py-5 flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-6 shrink-0">
              <p className="text-sm text-[#64748B]">{t.assignedTo || "—"}</p>
              <p className="text-sm text-[#64748B]">{t.dueDate || "—"}</p>
            </div>
          </div>
        ))}
      </div>
